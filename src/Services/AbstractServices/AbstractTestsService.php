...
class AbstractTestsService
{
    public static function get(TestsQueryFilter $filter = null, array $params = []) : Collection|LengthAwarePaginator
    {
        ...
        */
        if($filter == null) {
            $filter = new TestsQueryFilter($request);
        }
    ...
    {
        $object = Tests::where('uuid', $objectId)->first();

        $action = '\\NextDeveloper\\Support\\Actions\\Tests\\' . Str::studly($action);
